...
$api_client.interceptors.response.use(config => {
    return config
}, (async(error) => {
    ...
    if (originalRequest && !originalRequest._isRetry && error.response.status === 401)
    {
        ...
        try {
            const response = await axios.post(`${process.env.REACT_APP_BASE_URL_CLIENT}/update_tokens`, {refresh: getCookie('ctok')})
...
export async function get_patients(limit:number){
   await $api_client.get(`${process.env.REACT_APP_BASE_URL_DOCTOR}/patients?limit=${limit}`).then((r) =>{
       if(r.status === 200){
            return r.data
       }else{
